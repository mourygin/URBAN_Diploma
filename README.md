...
В ходе разработки программы применялось лишь ручное тестирование. В результате код не оказался свободным от ошибок. Уже после загрузки программы на github.com, и даже после предзащитной проверки было выявлено две ошибки. Правильнее сказать – одна ошибка и одна возможность совершения ошибки оператором.<br>

Ошибка заключалась в том, что при выдаче покупателю акционного подарка количество данных игрушек на складе не уменьшалось. Ошибка была легко исправлена.<br>

Второй случай был связан с отсутствием проверки соотношения значений параметров first_date и last_date в модели Action. Такая проверка была добавлена в метод класса __init__.<br>
...
Тем не менее данный проект может быть доработан до состояния работоспособности и востребованности на реальном рынке. Предсказывать трудно, особенно будущее… Может быть данная работа послужит выполнению реального заказа в качестве прототипа.
